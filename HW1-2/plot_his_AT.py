...
def grab_AThis(dataframe_list, sys_argv, mode):
    his = []
    his_label = []
    for i in range(len(dataframe_list)):
        if i == 0 and mode == 'grad':
            ...
            his.append(from_dataframe(dataframe_list[i].iloc[:, 5]))
...
if __name__ == '__main__':
    ...
    his, his_label = grab_AThis(dataframe_list, args, 'grad')
    TrainHistoryPlot(his, his_label, sys.argv[1] + '_grad_norm', 'AT_grad_norm', ['epoch', 'grad_norm'])
    his, his_label = grab_AThis(dataframe_list, args, 'loss')
    TrainHistoryPlot(his, his_label, sys.argv[1] + '_loss', 'AT_loss', ['epoch', 'loss'])
    his, his_label = grab_AThis(dataframe_list, args, 'acc')
