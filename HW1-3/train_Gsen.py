...
#-------------------------------------------------------------------------------
#train_Gsen.py is the training process of cifar-10 dataset for MLDS homework 1-3:
#Gerneralization vs Sensitivity
#--------------------------------------------------------------------------------
def TrainModel(model, saving_name, epochs, lr, batch_size, device, save = True):
    if device < 0:
    ...
    history = ['Epoch,Train Loss,Train Acc.,Sensitivity,Test Loss,Test Acc.\n']
    ...
    for epoch in range(epochs):
        ...
        for _iter, train_data in enumerate(train_eval_loader):
            ...
            sen.append(train_x.grad.norm() / train_x.grad.numel())
...
if __name__ == '__main__':
    ...
    model = SenCNN()
    TrainModel(model, sys.argv[1], int(sys.argv[2]), float(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]))
